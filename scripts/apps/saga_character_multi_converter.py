...
STARSHIP_MANEUVERS_JSON_PATH = os.path.join(LOCAL_ROOT_PATH, r'data\index_elements\starship_maneuvers.json')
...
PROCESS_CONFIG = {
    "species": True, # Enable species processing
    "skills": False,  # Enable skills processing
    "feats": False,
    "talents": False,
    "techniques": False,
    "secrets": False,
...
def species_parse_ability_modifiers(row_dict):
    ...
    if "add +2 to one ability" in species_traits_col_text and "Str " in species_traits_col_text:
    ...
    for csv_col, full_name in ability_map.items():
        ...
        if val_str:
            ...
            except ValueError: print(f"Warning (Species): Non-integer ability value '{val_str}' for {full_name} in species '{row_dict.get('SPECIES', 'Unknown')}'.")
    return modifiers
...
def species_add_trait_if_new(special_qualities_list, name, description, existing_names_set, defense_types_covered):
    ...
    if name_lower not in existing_names_set:
        ...
        trait_effects = parse_effects_from_text(processed_description, context_skill="Trait")
        ...
        for effect in trait_effects:
            if effect.get("type") == "defense_bonus":
                ...
                if bonus_target == "all": defense_types_covered.update(['reflex', 'fortitude', 'will']); break
                elif bonus_target in ['reflex', 'fortitude', 'will']: defense_types_covered.add(bonus_target)
        desc_lower = processed_description.lower()
        if "all of your defenses" in desc_lower and re.search(r"\+\d+", desc_lower): defense_types_covered.update(['reflex', 'fortitude', 'will'])
        else:
...
def process_species_csv():
    ...
    if raw_traits_data:
        for row in raw_traits_data:
            s_name = row.get('SPECIES', '').strip(); t_name = row.get('TRAIT', '').strip(); benefit = row.get('BENEFIT', '').strip()
    ...
    for row in raw_species_data:
        name = row.get('SPECIES', '').strip()
        if not name or not row.get('BOOK'): continue
        ...
        if "Droid," in name: trait_sources_to_check.append("Droid")
        for lookup_name in set(trait_sources_to_check):
        ...
        for def_col, (def_key, def_full_name) in defense_map.items():
            def_val = row.get(def_col, '').strip()
            if def_val:
                try:
                    bonus = int(def_val)
                    if def_key not in defense_types_covered:
                        species_add_trait_if_new(current_sq_list, f"Species Bonus to {def_full_name}", f"Gains a +{bonus} species bonus to {def_full_name}.", existing_trait_names, defense_types_covered)
                except ValueError: print(f"Warning (Species): Non-integer defense bonus '{def_val}' for {def_col} in '{name}'.")
        species_obj = {"name": name, "pc_or_npc": row.get('NPC or PC?', '').strip(), "ability_modifiers": species_parse_ability_modifiers(row), "size": species_parse_size(row.get('Size', '')), "speed_squares": species_parse_speed(row.get('Spd', '')), "special_qualities": sorted(current_sq_list, key=lambda x: x['name']), "automatic_languages": species_parse_languages(row), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
...
def process_skills_csv():
    ...
    for row in raw_def_data:
        skill_name = row.get('SKILLS', '').strip()
        if not skill_name: continue
        class_skills = []
        class_cols_to_check = ['Jedi', 'Noble', 'Scoundrel', 'Scout', 'Soldier', '  Scoundrel']
        for class_col_raw in class_cols_to_check:
            class_col_cleaned = class_col_raw.strip()
            if row.get(class_col_raw, '').strip().lower() == 'x' and class_col_cleaned not in class_skills:
                class_skills.append(class_col_cleaned)
        skills_base_data[skill_name] = {"name": skill_name, "key_ability": row.get('Ability', '').strip(), "armor_check_penalty_applies_generally": row.get('Armor check penalty?', '').strip().upper() == 'Y', "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip(), "class_skill_for": sorted(list(set(class_skills))), "common_uses": {}}
    raw_uses_data = read_csv_data(SKILL_USES_CSV_PATH)
    if raw_uses_data:
        for row in raw_uses_data:
            skill_name = row.get('Skill', '').strip(); use_name = row.get('Use', '').strip()
            if not skill_name or not use_name or skill_name not in skills_base_data: continue
            normalized_use = use_name.lower().strip()
            skills_base_data[skill_name]["common_uses"][normalized_use] = {"use_name": use_name, "source_book_for_use": skills_base_data[skill_name]["source_book"], "page_for_use": skills_base_data[skill_name]["page"], "related_ability_for_use": row.get('Ability', skills_base_data[skill_name]["key_ability"]).strip(), "trained_only": row.get('Trained', '').strip().upper() == 'Y', "armor_check_penalty_for_this_use": row.get('AP', '').strip().upper() == 'YES', "dc_details": row.get('DC', '').strip(), "notes": mtr_explanations(row.get('Notes', '').strip()), "requirements": None, "take_10_20_allowed": row.get('Take 10/20', '').strip(), "retry_info": mtr_explanations(row.get('Retry', '').strip()), "time_to_perform": row.get('Time', '').strip()}
    raw_actions_data = read_csv_data(SKILL_ACTIONS_CSV_PATH)
    if raw_actions_data:
        for row in raw_actions_data:
            skill_name = row.get('SKILL', '').strip(); action_name = row.get('ACTION', '').strip()
            if not skill_name or not action_name or skill_name not in skills_base_data: continue
            normalized_action = action_name.lower().strip()
            ...
            if use_obj:
                ...
                skills_base_data[skill_name]["common_uses"][normalized_action] = {"use_name": action_name, "source_book_for_use": row.get('BOOK', skills_base_data[skill_name]["source_book"]).strip(), "page_for_use": str(row.get('PAGE', skills_base_data[skill_name]["page"])).strip(), "related_ability_for_use": skills_base_data[skill_name]["key_ability"], "trained_only": row.get('Trained Only?', '').strip().lower() == 'x', "armor_check_penalty_for_this_use": None, "dc_details": None, "notes": None, "requirements": mtr_explanations(row.get('REQUIREMENTS', '').strip()), "take_10_20_allowed": None, "retry_info": None, "time_to_perform": None}
    final_skill_list = []
...
def process_feats_csv():
    ...
    for row in raw_data:
        name = row.get('FEAT', row.get('Feat', '')).strip()
        if not name: continue
        benefit_text = mtr_explanations(row.get('BENEFIT', row.get('Benefit', '')).strip())
        full_text_additional = mtr_explanations(row.get('Desc.', row.get('DESC', row.get('Desc', row.get('Description', '')))).strip()) or None
        prereq_text_raw = get_prerequisite_text_from_row_dict(row, ['PREREQUISITES', 'PREREQUISITE', 'Prerequisites', 'Prerequisite'])
        tags_str = row.get('TAG', '').strip()
        feat_types = []
        if tags_str:
            cleaned_tags = tags_str.replace('][', ',').replace('[', '').replace(']', '')
            feat_types = sorted([t.strip() for t in cleaned_tags.split(',') if t.strip()])
        bonus_classes = []
        base_class_columns = ["Jedi", "Noble", "Scoundrel", "Scout", "Soldier"]
        for char_class_col_name in base_class_columns:
            class_marker = row.get(char_class_col_name, '').strip()
            if class_marker: bonus_classes.append(char_class_col_name)
        primary_effect_text = benefit_text if benefit_text else full_text_additional
        feat = {"name": name, "type": feat_types if feat_types else ["Unknown"], "prerequisites_text": prereq_text_raw, "prerequisites_structured": parse_prerequisites_from_text(prereq_text_raw), "benefit_summary": benefit_text, "effects": parse_effects_from_text(primary_effect_text), "bonus_feat_for_classes": sorted(list(set(bonus_classes))), "source_book": row.get('BOOK', row.get('Source', '')).strip(), "page": str(row.get('PAGE', row.get('Page', ''))).strip(), "special_note": row.get('SPECIAL', row.get('Special', '')).strip() or None}
        if full_text_additional and full_text_additional != benefit_text : feat["full_text_description"] = full_text_additional
...
def process_talents_csv():
    ...
    for row in raw_data:
        ...
        primary_effect_text = benefit_text if benefit_text else full_text_additional
        talent = {"name": name, "talent_tree": row.get('TREE', 'Unknown').strip(), "class_association": associated_classes, "prerequisites_text": prereq_text_raw, "prerequisites_structured": parse_prerequisites_from_text(prereq_text_raw), "description": benefit_text, "effects": parse_effects_from_text(primary_effect_text), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
        if full_text_additional and full_text_additional != benefit_text : talent["full_text_description"] = full_text_additional
...
def process_techniques_csv():
    ...
    for row in raw_data:
        name = row.get('FORCE TECHNIQUE', '').strip()
        if not name: continue
        effect_text = mtr_explanations(row.get('EFFECT', '').strip())
        full_text_additional = mtr_explanations(row.get('Desc.', '').strip()) or None
        prereq_text_raw = get_prerequisite_text_from_row_dict(row, ['PREREQUISITE', 'Prerequisites'])
        context_skill = "Use the Force"
        primary_effect_text = effect_text if effect_text else full_text_additional
        technique = {"name": name, "force_power_association": row.get('Force Power Association', '').strip() or None, "prerequisites_text": prereq_text_raw, "prerequisites_structured": parse_prerequisites_from_text(prereq_text_raw), "effect_description": effect_text, "effects": parse_effects_from_text(primary_effect_text, context_skill), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
        if full_text_additional and full_text_additional != effect_text: technique["full_text_additional_desc"] = full_text_additional
...
def process_secrets_csv():
    ...
    for row in raw_data:
        name = row.get('FORCE SECRET', '').strip()
        if not name: continue
        effect_text = mtr_explanations(row.get('EFFECT', '').strip())
        full_text_additional = mtr_explanations(row.get('Desc.', '').strip()) or None
        prereq_text_raw = get_prerequisite_text_from_row_dict(row, ['PREREQUISITE', 'Prerequisites'])
        context_skill = "Use the Force"
        primary_effect_text = effect_text if effect_text else full_text_additional
        secret = {"name": name, "line_of_sight_requirement_text": row.get('LOS', '').strip() or None, "prerequisites_text": prereq_text_raw, "prerequisites_structured": parse_prerequisites_from_text(prereq_text_raw), "effect_description": effect_text, "effects": parse_effects_from_text(primary_effect_text, context_skill), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
        if full_text_additional and full_text_additional != effect_text: secret["full_text_additional_desc"] = full_text_additional
...
def process_regimens_csv():
    ...
    for row in raw_data:
        name = row.get('FORCE REGIMEN', '').strip()
        if not name: continue
        effect_text = mtr_explanations(row.get('EFFECT', '').strip())
        full_text_additional = mtr_explanations(row.get('Desc.', '').strip()) or None
        prereq_text_raw = get_prerequisite_text_from_row_dict(row, ['PREREQUISITE', 'Prerequisites'])
        lightsaber_col_val = row.get(' Lightsaber?', row.get('Lightsaber?', '')).strip().lower()
        ...
        primary_effect_text = effect_text if effect_text else full_text_additional
        regimen = {"name": name, "action_text": act_text or None, "target_info": row.get('Target', '').strip() or None, "is_lightsaber_related": True if lightsaber_col_val == 'yes' else (False if lightsaber_col_val == 'no' else None), "time_to_complete": row.get('Time', '').strip() or None, "prerequisites_text": prereq_text_raw, "prerequisites_structured": parse_prerequisites_from_text(prereq_text_raw), "effect_description": effect_text, "effects": parse_effects_from_text(primary_effect_text, context_skill), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
        if full_text_additional and full_text_additional != effect_text: regimen["full_text_additional_desc"] = full_text_additional
...
def process_starship_maneuvers_csv():
    ...
    for row in raw_data:
        name = row.get('STARSHIP MANEUVER', '').strip()
        if not name: continue
        effect_text = mtr_explanations(row.get('EFFECT', '').strip())
        descriptor_str = row.get('DESCRIPTOR', '').strip()
        descriptor_tags = []
        if descriptor_str:
            cleaned_descriptors = descriptor_str.replace('][', ',').replace('], [', ',')
        ...
        action_type_text = parse_action_type(row.get('Time', '').strip())
        context_skill = "Pilot"; skill_check_raw_text = row.get('CHECK', '').strip()
        ...
        maneuver = {"name": name, "descriptor_tags": descriptor_tags if descriptor_tags else [], "action_to_activate": action_type_text or "Unknown", "target_info": row.get('Target', '').strip() or None, "skill_check_raw_text": skill_check_raw_text or None, "effect_description": effect_text, "effects": parse_effects_from_text(effect_text, context_skill), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
...
def process_force_powers_csv():
    ...
    for row in raw_data:
        power_name_raw = row.get('FORCE POWER', '').strip()
        if not power_name_raw: continue
        name = power_name_raw; name_qualifier = None
        match_qualifier = re.search(r"\[([\w\s-]+)\]$", power_name_raw)
        if match_qualifier: name_qualifier = match_qualifier.group(1).strip(); name = power_name_raw[:match_qualifier.start()].strip()
        full_description_text = mtr_explanations(row.get('Desc.', '').strip())
        effect_summary_text = mtr_explanations(row.get('EFFECT', '').strip())
        descriptors = {"dark_light_affinity": row.get('   Dark/Light', '').strip() or None, "is_lightsaber_form_related": row.get('   Lightsaber form   ', '').strip().lower() == 'x', "is_mind_affecting": row.get(' Mind-affect.', '').strip().lower() == 'x', "is_telekinetic": row.get('  Telekinetic', '').strip().lower() == 'x', "is_unleashed_power": row.get('  Unleashed', '').strip().lower() == 'x'}
        if descriptors["dark_light_affinity"] and descriptors["dark_light_affinity"].lower() not in ['dark', 'light']: descriptors["dark_light_affinity"] = None
        power = {"name": name, "name_qualifier": name_qualifier, "full_text_description": full_description_text or None, "action_to_activate": parse_action_type(row.get('Action to activate', '').strip()), "action_to_maintain": parse_action_type(row.get('Action to maintain', '').strip()) or None, "target_info": row.get('Target', '').strip() or None, "range_squares": row.get('Sq.', '').strip() or None, "line_of_sight_requirement_text": row.get('LOS', '').strip() or None, "effect_summary_dc_progression": effect_summary_text, "effects": parse_effects_from_text(effect_summary_text, context_skill="Use the Force"), "descriptors": descriptors, "special_rules_and_notes": parse_special_rules_from_description(full_description_text), "source_book": row.get('BOOK', '').strip(), "page": str(row.get('PAGE', '')).strip()}
...
def process_prestige_classes_csv():
    ...
    try:
        if PRESTIGE_CLASSES_CSV_PATH and 'YOUR_PATH_TO_CSV' not in PRESTIGE_CLASSES_CSV_PATH and os.path.exists(PRESTIGE_CLASSES_CSV_PATH):
            with open(PRESTIGE_CLASSES_CSV_PATH, mode='r', encoding='utf-8-sig') as file:
                ...
                if first_line_peek_raw and first_line_peek_raw[0].startswith('\ufeff'):
                    first_line_peek_raw[0] = first_line_peek_raw[0][1:]
                if first_line_peek_raw: raw_rows_list.append(first_line_peek_raw)
                for row_content in reader: raw_rows_list.append(row_content)
    ...
    prereq_column_names = ["TRAINED SKILL PREREQUISITE", "SKILL FOCUS PREREQ.", "FEAT PREREQUISITE", "TALENT PREREQUISITE", "OTHER PREREQUISITE"]

    for i, row_list_raw in enumerate(raw_rows_list):
        row_list = [clean_value(cell) for cell in row_list_raw]

        if parsing_state == "EXPECTING_MAIN_HEADER":
            if "CLASS" in row_list and "HP" in row_list and ("PREREQUISITES" in row_list or "TRAINED SKILL PREREQUISITE" in row_list):
                main_header_map.clear() 
                for idx, header_text in enumerate(row_list):
                    main_header_map[header_text.strip()] = idx 
        ...
        if parsing_state == "EXPECTING_CLASS_DATA" or parsing_state == "PARSING_SUBSECTION_DATA":
            ...
            if book_idx != -1 and class_idx != -1 and \
               ...
               row_list[book_idx] and row_list[class_idx]:
                 is_new_class_row = True
        
        is_subsection_header_row = len(row_list) > 2 and row_list[2] and "Can use the following talent trees" in row_list[2]
        is_empty_line = all(not cell for cell in row_list)

        if is_new_class_row:
            if current_class_obj: processed_classes.append(current_class_obj)
            combined_prereq_text = get_prerequisite_text_from_row_list(row_list, main_header_map, prereq_column_names)
            current_class_obj = {
                "name": row_list[main_header_map.get('CLASS', 2)],
                "source_book": row_list[main_header_map.get('BOOK', 0)],
                "page": str(row_list[main_header_map.get('PAGE', 1)]),
                "hit_points_die": row_list[main_header_map.get('HP', 3)],
                "force_points_per_level": clean_value(row_list[main_header_map.get('Force Points', 4)]),
                "defense_bonuses": {"reflex": clean_value(row_list[main_header_map.get('Ref',5)]), "fortitude": clean_value(row_list[main_header_map.get('Fort',6)]), "will": clean_value(row_list[main_header_map.get('Will',7)])},
                "minimum_character_level_text": row_list[main_header_map.get('Min lvl', 8)],
                "prerequisites_summary": {key: (row_list[main_header_map[key]] if main_header_map.get(key, -1) < len(row_list) and main_header_map.get(key, -1) != -1 else None) for key in prereq_column_names},
            ...
            parsing_state = "PARSING_SUBSECTION_DATA"
                    ...
                    if next_row_peek_raw:
                        ...
                        if peek_book_idx != -1 and peek_class_idx != -1 and \
                           ...
                           next_row_peek_cleaned[peek_book_idx] and next_row_peek_cleaned[peek_class_idx]:
                            parsing_state = "EXPECTING_CLASS_DATA" 
                    else: parsing_state = "EXPECTING_CLASS_DATA" 
                except IndexError: parsing_state = "EXPECTING_CLASS_DATA"
                continue 

            tt_name_idx, tt_source_idx = 2, 4
            ...
            if talent_tree_name:
                ...
                if final_talent_tree_source.upper() == 'PRC' and current_class_obj:
            ...
            level_val = row_list[lvl_idx] if len(row_list) > lvl_idx else ''
